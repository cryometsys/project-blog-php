...
function installBlog(PDO $pdo) {
    ...
    if (!$error) {
        ...
        if ($sql === false) $error = 'Cannot find SQL file';
    }

    // Connect to the new database and try to run the SQL commands
    if (!$error) {
        $result = $pdo->exec($sql);
        if ($result === false) $error = 'Could not run SQL: ' . print_r($pdo->errorInfo(), true);
    }
...
function createUser(PDO $pdo, $username, $length = 10) {
    ...
    if(!$error) {
        //Create a hash of the password, to make a stolen User database worthless
        $hash = password_hash($password, PASSWORD_DEFAULT);
        if($hash == false) $error = 'Password hashing failed';
    }

    //Insert user details, including hashing password
    if (!$error) {
        $result = $stmt->execute(
            array(
                'username' => $username,
        ...
        if ($result === false) $error = 'Could not run the user password update';
    }
    if ($error) $password = '';
